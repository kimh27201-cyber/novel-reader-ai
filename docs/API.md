...
如果正文规则未命中，接口会返回 `400`，错误消息会包含章节 URL、`ruleContent.content` 和响应长度，便于判断是规则失效、地址错误还是站点返回异常。

### 写回章节正文缓存

`PATCH /api/chapters/{chapter_id}/content`
...
该接口只允许当前登录用户更新自己书架里的章节。写入成功后 `is_cached` 会变为 `true`，阅读器后续打开同一章节不再重复实时解析书源。
