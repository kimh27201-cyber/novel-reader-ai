...
4. 在“导入”页打开书源详情，说明 H5 兼容诊断会拆分规则兼容和网络测试。
5. 点击“批量检测”，展示正在测试 x/y、通过/失败/不兼容状态，以及失败源不会进入发现页。
6. 编辑一个书源名称或分组，展示分组统计、当前结果批量启停和删除确认。
7. “发现”页面查看当前可用书源数量，搜索“星轨”，展示本次使用了哪些书源。
8. 选择《星轨图书馆》，查看目录解析结果。
9. 加入云端书架后，进入“书架”页面确认书籍出现。
10. 进入阅读器，确认章节正文能打开。
11. 阅读器里触发 AI 总结或问答。
12. “我的”页面进入 AI 记录，查看总结、问答历史和调用日志。
...
> 前端没有盲目把所有导入源都拿去搜索，而是先做规则兼容诊断，再通过单源或批量检测确认网络可用性。发现页只使用启用、兼容、检测通过的书源，这样可以避免某个失败源拖慢整体搜索，也能把失败原因明确反馈给用户。
