...
4. 在“我的 > 关于”连续点击版本号 7 次开启调试模式，进入“开发与验收”。
5. 点击“一键演示准备”：
   - 预填演示账号 `student / secret123`。
   - 可复制后端启动命令。
   - 按清单完成书源、搜索、阅读器和 AI 记录演示。
   - 查看“本地阅读兜底”，确认已有内置书籍和 `static/test-novel.txt`，现场网络不稳定时先展示本地阅读链路。
   - 使用“真机验收”逐项记录验证进度，避免漏测。
6. 查看“APK 展示准备”卡片：
   - 前 3 项全部显示“已就绪”后，可以录屏演示主链路。
   - DCloud AppID 和云打包签名是 HBuilderX 发行阶段的手动项。
7. 使用合法演示书源或本地 TXT，完成书源导入、批量检测、发现页搜索、阅读器、AI 记录链路。
...
- 冷启动正常，底部四个 Tab 可切换。
- “我的”页默认不显示开发验收卡片，连续点击版本号 7 次后显示“开发与验收”。
- “我的”页后端地址保存、自检、登录正常，“APK 展示准备”前 3 项已就绪。
- “开发与验收”里“一键演示准备”可以预填演示账号、复制启动命令，并显示主链路演示清单。
- “本地阅读兜底”显示内置书籍、本地 TXT 示例和阅读器兜底均可用；不需要新增更多演示数据。
- “真机验收”清单可以逐项点选通过，最终显示 `11/11`。
