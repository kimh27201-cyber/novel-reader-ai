...
assert.match(voicePage, /apiClient\.getTtsStatus\(\)/)
assert.match(voicePage, /apiClient\.listTtsVoices\(\)/)
assert.match(voicePage, /未登录不影响设备声音和离线角色效果/)
assert.match(voicePage, /火山引擎凭据/)
assert.match(voicePage, /连接到了错误服务/)
assert.match(voicePage, /ADB reverse tcp:8765 tcp:8765/)
