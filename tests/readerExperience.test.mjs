...
assert.equal(prefs.ttsRate, 1)
assert.equal(prefs.ttsAutoNextChapter, true)
assert.equal(prefs.ttsVoiceProvider, 'system')
assert.equal(prefs.ttsVoiceId, '')
assert.equal(prefs.ttsVoiceName, '系统默认')
...
savePrefs({ ttsRate: 1.5, ttsAutoNextChapter: false })
const savedTtsPrefs = getPrefs()
assert.equal(savedTtsPrefs.ttsRate, 1.5)
assert.equal(savedTtsPrefs.ttsAutoNextChapter, false)

savePrefs({ ttsRate: 1.3, ttsAutoNextChapter: 'false' })
const normalizedTtsPrefs = getPrefs()
...
assert.doesNotMatch(reader, /pageTurnKey/)
...
assert.match(reader, /reader-page-settle/)
assert.doesNotMatch(reader, /reader-page-slide-forward/)
assert.doesNotMatch(reader, /reader-page-cover-forward/)
assert.doesNotMatch(reader, /@keyframes reader-page-settle[\s\S]{0,240}transform/)
assert.match(reader, /changeTextIndent/)
assert.match(reader, /toggleReadAloud/)
assert.match(reader, /createReadAloudController/)
assert.match(reader, /read-aloud-player/)
assert.match(reader, /isReadAloudParagraphActive/)
assert.match(reader, /handleReadAloudChapterComplete/)
assert.match(reader, /previousReadPosition/)
assert.match(reader, /this\.chapterIndex = previousReadPosition\.chapterIndex/)
assert.match(reader, /ttsAutoNextChapter/)
assert.match(reader, /skipReadAloudParagraph/)
assert.match(reader, /cycleReadAloudRate/)
assert.match(reader, /openVoiceSelector/)
assert.match(reader, /prefs\.ttsVoiceName/)
assert.doesNotMatch(reader, /slice\(0,\s*4000\)/)
