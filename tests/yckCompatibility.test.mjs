...
assert.equal(jsContentCompatibility.level, 'partialCompatible')
...
assert.equal(jsContentCompatibility.contentReadable, false)
assert.ok(jsContentCompatibility.unsupportedReasons.some(item => item.stage === 'content' && /@js/i.test(item.reason)))

const preview = buildImportPreview([jsContentSource], [])
assert.equal(preview.incompatible, 0)
assert.equal(preview.partialCompatible, 1)
assert.equal(preview.sources[0].action, 'import')
assert.equal(preview.sources[0].compatibleLevel, 'partialCompatible')
