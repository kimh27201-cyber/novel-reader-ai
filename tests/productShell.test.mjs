...
assert.match(profile, /morphAppTheme/)
assert.match(profile, /pendingThemeId/)
assert.match(profile, /persist: false/)
assert.match(profile, /themePreviewFrame/)
...
assert.match(search, /getSourceDiscoverySnapshot/)
assert.match(search, /getSourceLibraryPage/)
assert.match(search, /searchUnifiedBooks/)
...
assert.match(
  bookshelf,
  /<DBottomSheet[\s\S]*?:bottom-offset-rpx="140"[\s\S]*?panel-class="book-action-sheet"/,
...
assert.match(bookshelf, /<DBottomSheet/)
assert.match(bookshelf, /panel-class="book-action-sheet"/)
  const bottomSheet = readFileSync(new URL('../components/feedback/DBottomSheet.vue', import.meta.url), 'utf8')
  const modal = readFileSync(new URL('../components/feedback/DModal.vue', import.meta.url), 'utf8')
  assert.match(bottomSheet, /bottomOffsetRpx/)
  assert.match(bottomSheet, /acquireOverlayScrollLock/)
  assert.match(modal, /acquireOverlayScrollLock/)
assert.match(bottomSheet, /uni\.upx2px\(offsetRpx\)/)
assert.match(bottomSheet, /:style="panelStyle"/)
assert.match(bookshelf, /<DModal/)
assert.match(bookshelf, /panel-class="delete-confirm-dialog"/)
assert.match(bookshelf, /DBookCover/)
assert.match(bookshelf, /DEmptyState/)

assert.match(readme, /adb reverse tcp:8765 tcp:8765/)
assert.match(readme, /http:\/\/127\.0\.0\.1:8765/)
...
assert.match(reader, /:class="\[themeClass, readerEntryClass/)
assert.match(reader, /<DBottomSheet/)
assert.match(reader, /readerTransitionClass/)
assert.match(reader, /reader\.chapter\.render/)
...
assert.match(app, /animation: app-tab-page-enter 100ms/)
