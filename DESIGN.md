...
主题切换不只改变颜色；书架、书源、发现、阅读器控制区、我的页面和底部导航必须同步改变字体、卡片几何、信息标签与一个可识别的图形母题。主题 ID 保持稳定，用于本地偏好存储。

| ID | 主题 | 设计语言 | 专属记忆点 |
|---|---|---|---|
| `xuanye` | 玄夜 | 星图解码仪、青色信号线、等宽数据标签 | 章节进度的流动星轨 |
| `candy` | 糖果绘本 | 纸艺描边、错位硬阴影、贴纸卡片 | 书籍封面的圆形贴纸书签 |
| `sakura` | 樱雾少女 | 信笺留白、楷体短标题、丝带细节 | 列表卡片右上的丝带标记 |
| `cyber` | 量子蓝图 | 冷光网格、直角面板、终端数据刻度 | 搜索与书架的坐标网格 |
| `noirGold` | 黑曜金 | 私人藏书票、宋体标题、双层金线 | 当前阅读卡的典藏边框 |

动效只服务于主题信息：玄夜进度条可流动，糖果保留短促回弹，其余主题使用克制的过渡；系统开启“减少动态效果”时必须停用循环动效。
